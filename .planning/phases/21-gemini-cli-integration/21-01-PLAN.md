...
<task type="auto">
  ...
  <files>bin/install.js, hooks/gsd-statusline.js, GSD-STYLE.md, commands/gsd/plan-phase.md, commands/gsd/settings.md</files>
  <action>
...
NOTE: commands/gsd/ paths may have been renamed to commands/grd/ in GRD - if commits reference commands/gsd/, apply to commands/grd/ equivalent.
...
In the staged changes, update branding:
- Replace `get-shit-done` with `get-research-done` in all paths and messages
- Replace `/gsd:` with `/grd:` in all command references
- Replace `gsd` variable prefixes with `grd` (e.g., `gsdTools` -> `grdTools`)
...
Apply same branding updates if needed, then commit:
```bash
git add -A
...
```
  ...
  <verify>
```bash
# No GSD/get-shit-done branding remains
rg -ic 'get-shit-done' bin/install.js || echo "Clean"
# Should print "Clean" or return 0
