...
**Update frontmatter to fix artifact naming:**
```yaml
---
name: grd:architect
...
**Important:** Change `produces: [HYPOTHESES.md]` to `produces: [OBJECTIVE.md]` - this is the correct artifact name per CONTEXT.md.

**Use ## Phase markdown format** (consistent with explore.md pattern, NOT `<step>` XML):
...
Produces OBJECTIVE.md: `grep "produces.*OBJECTIVE" commands/grd/architect.md`
  </verify>
  <done>
/grd:architect command fully implemented with:
- Frontmatter corrected (produces: OBJECTIVE.md)
- ## Phase markdown format (consistent with explore.md)
...
**Execution flow with 8 steps using ## Step markdown format:**
...
- **Use Write tool to create .planning/OBJECTIVE.md**

```
Write(file_path=".planning/OBJECTIVE.md", content=populated_template)
```
...
Contains execution steps: `grep -c "^## Step" agents/grd-architect.md` returns 8
...
Contains explicit Write tool call: `grep "Write.*OBJECTIVE" agents/grd-architect.md`
  </verify>
  <done>
grd-architect agent created with:
- Role as research advisor (proposes, explains, accepts override)
- 8-step execution flow using ## Step markdown format
...
- [ ] Command frontmatter produces OBJECTIVE.md (not HYPOTHESES.md)
- [ ] Command uses ## Phase markdown format (consistent with explore.md)
- [ ] grd-architect agent created with conversational synthesis workflow
- [ ] Agent uses ## Step markdown format for execution flow
...
- [ ] OBJECTIVE.md generation wired to template with explicit Write tool call
</verification>
