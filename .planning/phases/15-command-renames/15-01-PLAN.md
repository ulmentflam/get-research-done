...
<task type="auto">
  ...
  <action>
    ...
    2. Update frontmatter name field in BOTH renamed files:
       - Change `name: grd:plan-phase` to `name: grd:design-experiment`
    ...
    3. Update ALL cross-references in ALL .md files under .claude/ and commands/:
       - Replace `/grd:plan-phase` with `/grd:design-experiment` everywhere
    ...
    5. Verify no orphan references remain:
       ...
       - grep -r "grd:plan-phase" . --include="*.md" should return 0 results
...
<task type="auto">
  ...
  <action>
    ...
    2. Update frontmatter name field in BOTH renamed files:
       - Change `name: grd:execute-phase` to `name: grd:run-experiment`
    ...
    3. Update ALL cross-references in ALL .md files under .claude/, commands/, get-research-done/:
       - Replace `/grd:execute-phase` with `/grd:run-experiment` everywhere
...
<task type="auto">
  ...
  <action>
    ...
    2. Update frontmatter name field in BOTH renamed files:
       - Change `name: grd:discuss-phase` to `name: grd:scope-experiment`
       - Update description to reflect "experiment" terminology (e.g., "Capture your vision for an experiment before planning")

    3. Update ALL cross-references in ALL .md files under .claude/, commands/, get-research-done/:
       - Replace `/grd:discuss-phase` with `/grd:scope-experiment` everywhere
